...
const Contact: React.FC<ContactProps> = ({ config, onSendMessage }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert(
      'Cảm ơn bạn đã gửi liên hệ! Chúng tôi đã nhận được tin nhắn và sẽ phản hồi sớm nhất có thể.'
    );
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 animate-in fade-in duration-700">
      <SEOManager
        title="Liên Hệ"
        description="Gửi tin nhắn hoặc liên hệ trực tiếp với Thịnh Phú Food để được hỗ trợ tốt nhất."
      />

      <section className="bg-green-700 py-16 md:py-24 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl" />
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-green-900/20 rounded-full translate-y-1/2 -translate-x-1/2 blur-3xl" />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
            {config.title}
          </h1>
          <p className="max-w-2xl mx-auto text-green-100 text-lg opacity-90 leading-relaxed font-medium">
            {config.description}
          </p>
        </div>
      </section>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-2xl shadow-green-900/5 border border-slate-100">
              <h2 className="text-2xl font-bold text-slate-900 mb-8 flex items-center">
                <span className="w-8 h-1.5 bg-green-600 rounded-full mr-3" />
                Thông tin liên lạc
              </h2>
            ...
            <div className="bg-green-800 p-8 rounded-[2.5rem] text-white shadow-xl shadow-green-900/10">
              ...
              <p className="text-green-100 text-sm mb-6 opacity-80">
                Cập nhật những tin tức và khuyến mãi mới nhất qua mạng xã hội.
              </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      required
                      type="text"
                      className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-green-500 focus:bg-white transition-all font-bold text-slate-900"
                ...
                <div>
                  ...
                  <textarea
                    required
                    rows={5}
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-green-500 focus:bg-white transition-all font-bold text-slate-900 resize-none"
                ...
                <button
                  type="submit"
                  className="bg-green-600 text-white px-10 py-5 rounded-2xl font-black flex items-center space-x-3 shadow-xl shadow-green-200 hover:bg-green-700 transition-all active:scale-95 group w-full sm:w-auto justify-center"
                >
